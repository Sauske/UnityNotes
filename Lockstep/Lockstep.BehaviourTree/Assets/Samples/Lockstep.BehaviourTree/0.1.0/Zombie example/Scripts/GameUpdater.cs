...
namespace AIToolkitDemo
{
    class GameUpdater : MonoBehaviour
    {
        ...
        void Awake()
        {
            _aiUpdater = delegate (AIEntity entity, float gameTime, float deltaTime)
                {
                    return entity.UpdateAI(gameTime, deltaTime);
                };
            _requestUpdater = delegate (AIEntity entity, float gameTime, float deltaTime)
                {
                    return entity.UpdateReqeust(gameTime, deltaTime);
                };
            _behaviorUpdater = delegate (AIEntity entity, float gameTime, float deltaTime)
                {
