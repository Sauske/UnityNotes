...
namespace EightQueen
{
    ...
    public class EightQueen : MonoBehaviour
    {
        ...
        List<int[]> GetSolutions(int count)
        {
            ...
            PutQueen(QUEEN_COUNT, queenList, 0);
        ...
        {
            for (int idx = 0; idx < nextY; idx++)
            {
                int x = Mathf.Abs(queenList[idx] - queenList[nextY]);
                int y = Mathf.Abs(idx - nextY);
                if (x == y || queenList[idx] == queenList[nextY])
                {
                    return true;
                }
            }
