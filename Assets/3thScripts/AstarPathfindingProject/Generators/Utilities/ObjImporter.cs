...
namespace Pathfinding
{
    public class ObjImporter
    {
        ...
        public static Mesh ImportFile(string filePath)
        {
            ...
            int colorsLen = newMesh.faceData.Length;
            Vector3[] newVerts = new Vector3[newMesh.faceData.Length];
            Color[] newColors = new Color[colorsLen];
            ...
            {
                int verIndex = (int)v.x - 1;
                newVerts[i] = newMesh.vertices[verIndex];
                newColors[i] = newMesh.colors[verIndex];
                if (v.y >= 1)
        ...
        public static Mesh Import3DFile(string fileData)
        {
#if NETFX_CORE
			throw new System.NotSupportedException("Method not available on this platform");
#else

            mesh3DStruct newMesh = createMesh3DStruct(fileData);
            populate3DMeshStruct(ref newMesh);
        ...
        /// <returns></returns>
        private static mesh3DStruct createMesh3DStruct(string entireText)
        {
#if NETFX_CORE
			throw new System.NotSupportedException("Method not available on this platform");
#else
            ...
            int vn = 0;
            int face = 0;
            mesh3DStruct mesh = new mesh3DStruct();
            mesh.entireText = entireText;
            using (StringReader reader = new StringReader(entireText))
            {
                string currentText = reader.ReadLine();
                    ...
                    else
                    {
                        ...
                        switch (brokenString[0])
                        {
                            ...
                            case "v":
                                ...
                                if (brokenString.Length >= 7)//如果后面3列有值，就是颜色
                                {
                                    mesh.colors[v] = new Color(System.Convert.ToSingle(brokenString[4]), System.Convert.ToSingle(brokenString[5]),
        ...
        /// </summary>
        /// <param name="mesh"></param>
        private static void populate3DMeshStruct(ref mesh3DStruct mesh)
        {
#if NETFX_CORE
			throw new System.NotSupportedException("Method not available on this platform");
#else
            ...
            string entireText = stream.ReadToEnd();
            stream.Close();*/
            using (StringReader reader = new StringReader(mesh.entireText))
            {
                string currentText = reader.ReadLine();
