...
namespace UnityWebSocket
{
    public class WebSocket : IWebSocket
    {
        ...
        public event EventHandler<ResEventArgs> OnMessage;
        ...
        {
            Debug.LogFormat("Connect Task Begin ...url:{0}", Address);
        ...
        private async Task ReceiveTask()
        {
            ...
            try
            {
                while (!isClosed)
                {
                   ...
                   // Array.Copy(ms.ToArray(), data, count);
                    ms.SetLength(0);
                    switch (result.MessageType)
                    {
                        ...
                        case WebSocketMessageType.Close:
                            ...
                            Debug.LogFormat("CloseNet:Address:{0},Code:{1},Reason:{2}", Address, closeCode, closeReason);
                            break;
                    }
                }
            }
...
#if !UNITY_WEB_SOCKET_ENABLE_ASYNC
            HandleEventSync(new ResEventArgs(opcode, rawData));
#else
        ...
        internal void Update()
        {
            ...
            while (eventQueue.Count > 0)
            {
                ...
                else if (e is ResEventArgs)
                {
                    OnMessage?.Invoke(this, e as ResEventArgs);
                }
        ...
        {
            Debug.Log($"<color=yellow>[UnityWebSocket]</color>" +
